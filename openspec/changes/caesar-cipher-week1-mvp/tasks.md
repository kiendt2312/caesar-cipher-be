...
- [ ] 10.13 Chạy lại full two-axis code review từ fixed point `dbc8d285c73756de58429be23f1af88abaaa7a89` tới accepted HEAD; Standards và Spec độc lập. Ba smell đã ghi ở `Deferred technical debt` là non-blocking. **Xong khi:** không còn hard Standards violation hay Spec finding chưa xử lý và báo cáo giữ hai axis riêng.
- [ ] 10.14 Sau khi toàn bộ evidence được chấp nhận, commit remediation trên isolated branch rồi push accepted HEAD tới remote `main` bằng refspec tường minh, chỉ khi quan hệ remote chứng minh fast-forward; tuyệt đối không force push và không archive OpenSpec. **Xong khi:** báo commit SHA, remote pre/post SHA và kết quả push chính xác; nếu remote đã tiến thì dừng để reconcile thay vì overwrite.
